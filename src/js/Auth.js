    ...
    return new Promise((resolve, reject) => {
      this.accessTitanKeyContractWith({
        ...
        method: (contractInstance, coinbase) => {
          return new Promise((resolve, reject) => {
            contractInstance.userUpdate(data.firstName, data.lastName, data.email, { from: coinbase, gas: 4444444 })
            .then((result) => {
              // Successful Sign-up
              resolve(data)
            })
  ...
  signup (state = null, data = {}) {
    return new Promise((resolve, reject) => {
      this.accessTitanKeyContractWith({
        ...
        method: (contractInstance, coinbase) => {
          return new Promise((resolve, reject) => {
            contractInstance.userSignUp(data.firstName, data.lastName, data.email, { from: coinbase })
            .then((result) => {
              // Successful Sign-up
              resolve(data)
            })
